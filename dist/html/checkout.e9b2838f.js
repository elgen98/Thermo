...
function addCardForm() {
    ...
    for(let i = cardItems.length - 1; i >= 0; --i)cardItems[i].className = "cardInfoShow";
}
...
function subtractQuantity(position) {
    productsInCart[position].quantity -= 1;
    if (productsInCart[position].quantity === 0) productsInCart[position].quantity = 1;
...
function calculateTotalPrice(position) {
    productsInCart[position].totalPrice = productsInCart[position].price * productsInCart[position].quantity;
...
function printTotal() {
    ...
    for(let i = 0; i < productsInCart.length; i++){
        ...
        if (totalAmount === 0) document.getElementById("summaryDiv").innerHTML = "";
        else amountDiv.innerHTML = totalAmount.toString() + " kr";
    }
}
function shopmore() {
    localStorage.clear();
...
window.onclick = function(event) {
    if (event.target == checkoutModal) checkoutModal.style.display = "block";
};

},{}]},["h9O60","fFAhY"], "fFAhY", "parcelRequire2445")
